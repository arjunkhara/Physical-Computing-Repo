...
#### GOLDSMITHS MARS ROVER: CODE SNIPPETS AND PROGRAMMING

The following code snippets (below) detail the programming I have done for the voice-recognition and commands Mars Rover. The code snippets are for all the existing modules that are on the rover. The rover also has additional upgradable modules which can be included, or existing sensors swapped out for new ones. I have included the code snippets for all of the modules that I have used while building the rover. However, even more modules can be added to the rover, given the availability of pins 14 to 18 (please refer to the I2C section on this page). I have left six additional module components on the chassis free for any of these additional extensions. 10 sensors and actuators are currently being used on this Mars Rover, with space for an additional six, making a total of 16 transducers for the rover. Where feasible and for ease of reading, I have combined multiple code snippets for these various sensors into a single snippet (e.g. the motor codes together with the Bluetooth module codes, and the proximity sensor codes together with the LED warning system codes). Note also that I have attempted to point out (as far as possible) each code snippet’s customisable module for use in other projects.
...
Below is basic code for the heartbeat sensor for the Goldsmiths Rover, which can be adjusted for individual project use.
...
#### GOLDSMITHS MARS ROVER: CONCLUDING THOUGHTS
Building a voice-recognition robot is the culmination of ten weeks' worth of learning and development in a field which, when I started out, had zero idea. The concept of physical computing was alien, an other-worldly subject that I might be able to appreciate from far. Which is why this robot means more to me than a university project, and which is why I tried to go way beyond the requirements of a distinction, and create something that I could not only keep beyond the course but motivate me to realise an actual academic and career path. Building this rover was by no means easy - far from it, but the sheer enjoyment of putting together something of this complexity and advancement was the biggest driver of pursuing this big hairy goal. And the most amazing part was that everything that made the rover work was planned, built, cut, assembled, wired, and programmed entirely by someone like me - a person with no background in engineering or electronics but a lot of enthusiasm to learn, just because it's cool to build stuff.
